...
export default function AgentEconomyPanel() {
  ...
  const tiles = [
    { label: "Earned (x402)", value: usdc(stats.earnedUsd), sub: `${stats.callsServed} calls served` },
    { label: "Scout spent", value: usdc(stats.spentUsd), sub: `${stats.callsPaid} calls paid` },
    {
      label: "Budget left today",
      value: usdc(stats.budgetRemainingUsd),
      sub: `of ${usdc(stats.dailyCapUsd)} cap`,
    },
    { label: "Net", value: usdc(stats.earnedUsd - stats.spentUsd), sub: "earned − spent" },
  ];
